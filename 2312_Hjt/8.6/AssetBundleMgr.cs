...
public class AssetBundleMgr : Editor
{
    ...
    public static void CreateAssetBundle()
    {
        ...
        StringBuilder sb = Res_Pack_Handle();
    ...
    /// 资源的打包划分（AB包打包策略）
    /// </summary>
    public static StringBuilder Res_Pack_Handle()
    {
        //打包资源路径
        ...
        foreach (var filePath in res_File_Path_Arr)
        {
            ...
            if (extensionName == ".prefab")
            {
                ...
                //就是资源名(资源清单)
                versionResName = "prefab/" + Path.GetFileNameWithoutExtension(filePath).ToLower();
            }
            else if (extensionName == ".mat")
            {
                ai.assetBundleName = Application.version + "/Mat/" + Path.GetFileNameWithoutExtension(filePath);
                versionResName = "mat/" + Path.GetFileNameWithoutExtension(filePath).ToLower();
            }
